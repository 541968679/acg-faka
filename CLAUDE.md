...
`deploy.sh` 会自动：打包（排除 vendor/runtime/.git 等）→ SCP 上传 → 解压覆盖 → 修正权限（www-data）→ 清 Smarty 缓存 → 重启 PHP-FPM。

### 服务器信息

| 组件 | 详情 |
|------|------|
| **GCP 项目** | `myvps-487409`，账号 `zengli3082@gmail.com` |
| **VM 实例** | `faka-us`，`us-west1-b`，`e2-micro`，外部 IP `34.82.229.217` |
| **OS** | Debian 12 (Bookworm) |
| **Web Server** | Nginx 1.22.1 → PHP 8.2 FPM（unix socket） |
| **Database** | MariaDB 10.11.14，数据库名 `acgfaka`，表前缀 `acg_`，用户 `faka` |
| **域名** | `shop.kaynlab.com`（Cloudflare 代理） |
| **项目路径** | `/var/www/acg-faka`（owner: `www-data`） |

### SSH 访问与运维

```bash
gcloud compute ssh faka-us --project=myvps-487409 --zone=us-west1-b

# 重启服务
sudo systemctl restart nginx php8.2-fpm mariadb

# 查看日志
sudo tail -f /var/www/acg-faka/runtime.log

# 生产数据库直连
mysql -u faka -p acgfaka
```
